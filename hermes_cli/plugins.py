...
class PluginContext:
    ...
    def register_required_hook(
        ...
        plugin_id = self.manifest.key or self.manifest.name
        entry = (plugin_id, callback)
        self._manager._required_hooks.setdefault(hook_name, []).append(entry)
        handle = self._track(
            "required_hook", hook_name,
            lambda: self._manager._remove_callback(
                self._manager._required_hooks, hook_name, entry
            ),
        )
        logger.debug(
        ...
        try:
            ...
            if register_fn is None:
                ...
                loaded.required_hooks_registered = [
                    registration.key
                    for registration in registrations
                    if registration.kind == "required_hook"
                ]
