...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "model", "monitoring", "pairing", "pause", "persist-smoke", "pets", "plugins", "portal", "profile",
        "project", "proxy",
        "prompt-size",
        "resume",
        "send", "sessions", "setup",
...
def _prepare_agent_startup(args) -> None:
    ...
    try:
        ...
        if _get_required_plugins():
            # Mandatory enforcers configured: discovery must be synchronous
            # so a missing/failed/misregistered enforcer aborts startup
            # instead of racing it in a daemon thread.
            discover_plugins()
        else:
